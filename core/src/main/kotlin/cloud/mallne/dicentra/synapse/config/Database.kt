...
fun Application.configureDatabase(vararg tables: Table = arrayOf(APIDBService.APIServiceData, ScopeService.Scopes)) {
    ...
    launch {
        ...
        if (config.data.autoCreateDelta) {
            ...
            databaseService {
                migration(config = config, tables = tables)
            }
        }
...
private suspend fun migration(config: Configuration, vararg tables: Table) {
...
private fun flyway(config: Configuration): MigrateResult =
    Flyway.configure()
        ...
        .locations(*config.data.migrationDirectory.map { "filesystem:$it" }.toTypedArray())
        .load()
        .migrate()